...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      updatePointerDownData(pointer, -1, local.posX, local.posY);
      clickSplat(pointer);
    };
    ...
    function handleFirstMouseMove(e: MouseEvent) {
      ...
      const color = generateColor();
      updateFrame();
      updatePointerMoveData(pointer, local.posX, local.posY, color);
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const local = getLocalPointer(touches[i].clientX, touches[i].clientY);
        if (!local.inside) continue;
        updateFrame();
        updatePointerDownData(pointer, touches[i].identifier, local.posX, local.posY);
      }
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const local = getLocalPointer(touches[i].clientX, touches[i].clientY);
        if (!local.inside) continue;
        updatePointerMoveData(pointer, local.posX, local.posY, pointer.color);
      }
    };
  ...
  return (
    <div
      className={['splash-cursor', className].filter(Boolean).join(' ')}
      style={{
        position: 'absolute',
        inset: 0,
        zIndex: 0,
